    ...
    def __init__(self):
        ...
        if raw:
            try:
                ...
                print("Loaded API_KEYS from ECS secret")
            except Exception as e:
                print(f"Failed to parse API_KEYS as JSON: {str(e)}")
        ...
        for key in self.REQUIRED_KEYS:
            if not self.api_keys.get(key):
                ...
                if env_val:
                    ...
                    print(f"Loaded {key} from individual env var")
        ...
        print("API keys loaded:", {k: v[:6] + "..." for k, v in self.api_keys.items()})
...
class ModelLoader:
    ...
    def __init__(self):
        if os.getenv("ENV", "local").lower() != "production":
            load_dotenv()
            print("Running in LOCAL mode: .env loaded")
        else:
            print("Running in PRODUCTION mode")

        self.api_key_mgr = ApiKeyManager()
        self.config = load_config()
        print("YAML config loaded:", list(self.config.keys()))
    ...
    def load_llm(self):
        ...
        if provider_key not in llm_block:
            print(f"LLM provider not found in config: {provider_key}")
        ...
        print(f"Loading LLM: provider={provider}, model={model_name}")
